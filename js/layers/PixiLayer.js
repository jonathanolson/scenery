...
define( function( require ) {
  ...
  scenery.PixiLayer = function PixiLayer( args ) {
    ...
    this.g = new PIXI.DisplayObjectContainer();
    ...
    domElement.width = this.logicalWidth * this.backingScale;
    domElement.height = this.logicalHeight * this.backingScale;
    domElement.style.width = this.logicalWidth + 'px';
    domElement.style.height = this.logicalHeight + 'px';
...
//    renderer.view.style.zIndex = '999';
...
//
//      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' addInstance: ' + trail.toString() );
//
//      assert && assert( !( trail.getUniqueId() in this.idFragmentMap ), 'Already contained that trail!' );
//      assert && assert( trail.isPainted(), 'Don\'t add nodes without isPainted() to PixiLayer' );
//
...
//      var trail = instance.trail;
//
//      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' removeInstance: ' + trail.toString() );
...
//      this.g.referenceCount--; // since we don't go down to the base group, adjust its reference count
    ...
    insertGroupIntoParent: function( group, parentGroup, subtrail ) {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' insertGroupIntoParent subtrail:' + subtrail.toString() );
    ...
    updateNode: function( node, fragment ) {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' updateNode: ' + node.constructor.name + ' #' + node.id );
    ...
    updateNodeGroup: function( node, group ) {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' updateNodeGroup: ' + node.constructor.name + ' #' + node.id );
...
//      }
    ...
    markBaseTransformDirty: function( changed ) {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' markBaseTransformDirty' );
    ...
    baseNodeInternalBoundsChange: function() {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' baseNodeInternalBoundsChange' );
      if ( this.cssTransform ) {
    ...
    updateBaseTransform: function( includesBaseTransformChange ) {
      sceneryLayerLog && sceneryLayerLog( 'PixiLayer #' + this.id + ' updateBaseTransform' );
